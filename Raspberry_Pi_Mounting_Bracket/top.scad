...
module floor()
{
  ...
  difference() {
    translate([-2,0,0]) cube([base_x, base_y, floor_thickness]);
...
module peg()
{
  cylinder(h=base_z-peg_cap, r=peg_or);
  translate([0,0,base_z-peg_cap])cylinder(h=peg_cap, r=peg_ir-0.5);

}

module peg_negative()
{
    cylinder(h=base_z, r=peg_pilot_ir);
}
...
difference() 
{
  union() 
  {
    //translate([board_dx, base_y-board_dy, board_dz+base_z])rotate([180,0,0]) %board();
    base();
    //%base_volume();
    pegs();
  }
